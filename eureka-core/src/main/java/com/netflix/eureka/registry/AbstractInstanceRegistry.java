...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
     ...
     * 读写锁: 进入读锁(不能有写锁), 进入写锁(不能有写锁, 不能有读锁).
     */
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        ...
        try {
            ...
            // 2. 如果gMap的租约存在就校验一下哪个更新, 用最新的lease
            ...
            // 4. 把注册动作存在一个recent队列里.
            recentRegisteredQueue.add(new Pair<Long, String>(
                    System.currentTimeMillis(),
                    registrant.getAppName() + "(" + registrant.getId() + ")"));

            // 5. 如果instanceInfo的OverriddenStatus改变了, ???? 这是什么
            ...
            // 6. 如果instanceInfo是要上线, 那么就更新租约起始时间.
    ...
    protected boolean internalCancel(String appName, String id, boolean isReplication) {
        ...
        try {
            ...
            // 2. 把<app-instanceId>添加到recentCanceledQueue.
            ...
            if (leaseToCancel == null) {
                ...
                // 3. 下线: 设置驱逐时间戳: evictionTimestamp
                leaseToCancel.cancel();
                InstanceInfo instanceInfo = leaseToCancel.getHolder();
                String vip = null;
                String svip = null;
                if (instanceInfo != null) {
                    // 4. 把instanceInfo添加到recentChangedQueue.里面
    ...
    public boolean renew(String appName, String id, boolean isReplication) {
        ...
        // 1. 拿到注册表的这个app的leaseMap
        Map<String, Lease<InstanceInfo>> gMap = registry.get(appName);
        Lease<InstanceInfo> leaseToRenew = null;
        if (gMap == null || (leaseToRenew = gMap.get(id)) == null) {
            RENEW_NOT_FOUND.increment(isReplication);
            logger.warn("DS: Registry: lease doesn't exist, registering resource: {} - {}", appName, id);
            return false;
        }
        // 2. 拿到instanceInfo, 做一些操作...
        ...
        renewsLastMin.increment(); // 计数
        // 3. 真正的续约, 更新约期的时间戳+90s
        leaseToRenew.renew();
        return true;
    }
